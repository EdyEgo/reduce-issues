import { onSnapshot, collection,getDocs,getDoc,doc,where,orderBy , query ,limit , startAt ,DocumentSnapshot } from 'firebase/firestore';

import {db} from '../../../firebase'
 

export const getAllDocumentsWithPagination = async ({itemsLimitedAt,path,orderByKey}:{path:string,orderByKey?:string,itemsLimitedAt:number})=>{
// registeredAt
 ...
 // example in a team we must check if the user is in members so whereFilter will be "membersId.memberId"
   ...
   collectionRef = query(collectionRef, orderBy(orderItemsBy) , limit(itemsLimitedAt));
...
export const getDocumentsWithSubscription =  ({path,valuesToIncludeInResult,callbackDocuments,orderByKey,wherePropertyKey,whereValues}:{callbackDocuments:(documents:{data?:any[],error:boolean,message?:string,unsub:()=>void,[key:string]:any})=>void,wherePropertyKey?:string,whereValues?:string[],path:string, orderByKey?:string,valuesToIncludeInResult?:{[key:string]:any}})=>{
  ...
  if(wherePropertyKey && whereValues)   collectionRef = query(collectionRef, where(wherePropertyKey,'in',whereValues), orderBy(orderItemsBy));
  if(wherePropertyKey == null && whereValues == null)collectionRef = query(collectionRef, orderBy(orderItemsBy));
    ...
    const unsub = onSnapshot(collectionRef,
      (snapshot:any)=>{
        ...
        if(valuesToIncludeInResult){
          callbackDocuments({error:false,data:documents,unsub,...valuesToIncludeInResult})
          return 
        }
        callbackDocuments({error:false,data:documents,unsub})
      },(error)=>{
    
        callbackDocuments({message:error.message,error:true,unsub,...valuesToIncludeInResult})
      }
    )
}
