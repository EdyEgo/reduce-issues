...
export async function createNewWorkspace(workspaceName:string,userObject:any,batch?:any){
    ...
    try{ 
      ...
      const workspaceURL = worspaceNameLowerCase.indexOf(' ') !== -1 ? worspaceNameLowerCase.split(' ').join('') : worspaceNameLowerCase
     ...
     if(batch){
      ...
      name:workspaceTrimed,photoURL:null,identified:createIdentified ,
      labels:[{name:'Feature',icon:'labelFeature',},{name:'Improvement',icon:'labelImprovement'},{name:'Bug',icon:'labelBug'} ], timezone:browserDate , workspaceURL , 
      membersId:{[userObject.uid]:{role:'Owner',invitedAt:serverTimestamp()}}
    }
      ,useAddDocument:true,useBatch:batch}) 
      
      
      await postNewDocument({collectionSelected:'users',documentName:userObject.uid,
      inputObject:{ 
        
        
        workSpaces:{[createdWorkSpace.id]:{ role:'Owner' }},
        workSpaceSelected:{id:createdWorkSpace.id} // maybe add this one tes'} 
      },noRegister:true,useBatch:batch}) 
