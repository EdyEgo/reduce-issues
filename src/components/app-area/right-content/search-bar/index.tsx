...
const SearchBar: React.FC<SearchBarProps> = () => {
    ...
    const workspaceSelected = useSelector((state:any)=>state.workspace.selectedWorkSpace) // workspaceURL
  ...
  function handleEnterKeyDown(event:any){
      ...
      if(event.key === "Enter" && searchText !== ""){
          ...
          navigate(`/${workspaceURL}/search?q=${searchText}`)
    ...
    return (  
      ...
      <div className="search-container px-4 mb-4 flex items-center gap-1">
        ...
        value={searchText}
        onChange={(event)=>{setSearchText(event.target.value)}} onKeyDown={handleEnterKeyDown}/>
        <CancelText onClick={deleteAllText} className={ searchText.length <=0  ? "invisible" : 'visible cursor-pointer'} sx={{height:20,width:20}}/>
      </div>
