...
export default function CheckboxListSecondary({
  ...
  function handleSettingFiltersIssues({
    ...
    if (
      ...
      dispatch(
        removeUserAtUnkownedIndex({ item: removeItem, type: checkboxType })
      );
      return;
    }
    ...
    if (removeItem != null && deselected) {
      ...
      dispatch(
        removeItemAtUnkownedIndex({ item: removeItem, type: checkboxType })
      );
      return;
    }
  }
  ...
  const handleToggle = (itemClicked: any, value: number) => () => {
    ...
    const currentItem = filtersIssuesStore[checkboxType].find(
      (item: any) => item.value.name === itemClicked.name
    );
  ...
  function returnFilteredMember(assigned: boolean) {
    ...
    const createdElements = teamMembers.map((valueMember, index) => {
      ...
      const findStoredValue = storeSelected.find(
        (item: any) => item.value.id === findMemberObjectInWorkspace.id
      );
      ...
      return returnElementOption({
        ...
        id: findMemberObjectInWorkspace.id,
      ...
      status: () => {
        const createdElements = statusList.map(({ icon, name }, index) => {
          const associatedNumber =
            ...
            selectedTeamObject.createdStatus[icon]
              ? selectedTeamObject.createdStatus[icon]
              : null;
      ...
      priority: () => {
        const createdElements = priorityList.map(({ icon, name }, index) => {
          const associatedNumber =
            ...
            selectedTeamObject.createdPriority[icon]
              ? selectedTeamObject.createdPriority[icon]
              : null;
          // availableItems.push({icon,name,checked:false,associatedNumber})
      ...
      labels: () => {
        const createdElements = labelsList.map(({ icon, name }, index) => {
          const associatedNumber =
            ...
            selectedTeamObject.createdLabel[icon]
              ? selectedTeamObject.createdLabel[icon]
              : null;
          // availableItems.push({icon,name,checked:false,associatedNumber})
          const findStoredValue = storeSelected.find(
