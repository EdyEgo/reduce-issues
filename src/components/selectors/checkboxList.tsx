...
export default function CheckboxListSecondary({
  ...
  const filtersIssuesStore = useSelector(
    (state: any) => state.filtersIssues.filtersListOrder
  ); // verifi if there is a selected property in  store

  const teamsList = useSelector(
    // we are gonna use this list both for the assignee and the creator
  ...
  function findTeamId(teamList: any[]) {
    ...
    return teamList.find(
      (team) => team.identified.toLowerCase() === params.teamURL
    ).id;
  }
  ...
  function handleSettingFiltersIssues({
    deselected,
    dueDate,
    assigneeId,
    creatorId,
    ...
    if (
      (deselected && checkboxType === "assignee") ||
      (deselected && checkboxType === "creator")
    ) {
      const filterItemObject = {
        is: true,
        value: returnFitFilterValueByIndexAndcheckboxType(filterIndex),
      };

      dispatch(
        removeItemAtUnkownedIndex({
          item: filterItemObject,
          type: checkboxType,
        })
      );
      return;
    ...
    if (checkboxType === "assignee" || checkboxType === "creator") {
      const userObject =
        returnFitFilterValueByIndexAndcheckboxType(filterIndex);
      const filterItemObject = {
  ...
  function returnFilteredMember(assigned: boolean) {
    ...
    if (items.length > 0) {
      return items.map((assignedUserObject: any, index: number) => {
        const findStoredValue = storeSelected.find(
          (user: any) => user.userId === assignedUserObject.id
        );

        const checked = findStoredValue?.is;
        if (checked != undefined)
          return returnElementOption({ ...assignedUserObject, index, checked });
    ...
    const createdElements = teamMembers.map((valueMember, index) => {
      ...
      const findMemberObjectInWorkspace = workspaceMembers.find(
        (member: any) => member.id === memberId
      );

      let associatedNumber = null;

      if (assigned && selectedTeamObject?.assignedIssues)
        associatedNumber = selectedTeamObject.assignedIssues;

      if (!assigned && selectedTeamObject?.issuesNumber)
        associatedNumber = selectedTeamObject.issuesNumber;

      availableItems.push({
        ...findMemberObjectInWorkspace,
        checked: false,
        associatedNumber,
      });

      return returnElementOption({
        index,
        name:
          findMemberObjectInWorkspace.firstName +
          " " +
          findMemberObjectInWorkspace.lastName,
        firstName: findMemberObjectInWorkspace.firstName,
        lastName: findMemberObjectInWorkspace.lastName,
        photoURL: findMemberObjectInWorkspace.photoURL,
        checked: false,
        associatedNumber,
      });
    });
    ...
    const listTypes: { [key: string]: any } = {
      status: () => {
        ...
        const createdElements = statusList.map(({ icon, name }, index) => {
          const associatedNumber =
            selectedTeamObject?.createdStatus &&
            selectedTeamObject.createdStatus[name]
              ? selectedTeamObject.createdStatus[name]
              : null;
          availableItems.push({ icon, name, checked: false, associatedNumber });
          const findStoredValue = storeSelected.find(
            (item: any) => item.value.name === name
          );
      ...
      priority: () => {
        if (items.length > 0) {
          return items.map((objectItem: any, index: number) => {
            const findStoredValue = storeSelected.find(
              (item: any) => item.value.name === objectItem.name
            );

            const checked = findStoredValue?.is;
            if (checked != undefined)
              return returnElementOption({ ...objectItem, index, checked });
        ...
        const createdElements = priorityList.map(({ icon, name }, index) => {
          const associatedNumber =
            selectedTeamObject?.createdPriority &&
            selectedTeamObject.createdPriority[name]
              ? selectedTeamObject.createdPriority[name]
              : null;
          availableItems.push({ icon, name, checked: false, associatedNumber });
      ...
      labels: () => {
        ...
        const createdElements = labelsList.map(({ icon, name }, index) => {
          const associatedNumber =
            selectedTeamObject?.createdLabel &&
            selectedTeamObject.createdLabel[name]
              ? selectedTeamObject.createdLabel[name]
              : null;
          availableItems.push({ icon, name, checked: false, associatedNumber });
          const findStoredValue = storeSelected.find(
  ...
  function returnElementOption(item: {
    checked: boolean;
    icon?: string;
      ...
      >
        <ListItemButton key={index + 2}>
          <ListItemAvatar key={index + 3}>
            {photoURL && <Avatar alt={`name`} src={photoURL} />}
            {icon && ExtractFitIconNoDinamic({ iconName: icon, index })}
            {photoURL === null && firstName && (
              <div className="icon-placeholder rounded-full" key={index + 4}>
                <span className="first-name-letter" key={index + 5}>
                  {firstName[0]}
                </span>
                {lastName && (
                  <span className="last-name-letter" key={index + 6}>
                    {lastName[0]}
                  </span>
                )}
              </div>
          ...
          {firstName == null && (
            <ListItemText id={name} primary={name} key={index + 7} />
          )}
          {firstName && lastName == null && (
            <ListItemText id={firstName} primary={firstName} key={index + 8} />
          )}
          {firstName && lastName && (
            <ListItemText
              key={index + 9}
